...
public class EditorActionMenuOverview extends AbstractStaticMenu {
	...
	public EditorActionMenuOverview(CosmeticsOG core, EditorMenuManager menuManager, Player owner, MenuCallback callback) {
		...
		this.inventory = Bukkit.createInventory(null, 27, Utils.legacySerializerAnyCase(Message.EDITOR_ACTION_OVERVIEW_MENU_TITlE.getValue()));
	...
	protected void build() {
		...
		ItemStack leftActionItem = ItemUtil.createItem(CompatibleMaterial.GUNPOWDER.getMaterial(), 1, Message.EDITOR_ACTION_OVERVIEW_MENU_SET_LEFT_CLICK.getValue());
		...
		ItemStack rightActionItem = ItemUtil.createItem(CompatibleMaterial.GUNPOWDER.getMaterial(), 1, Message.EDITOR_ACTION_OVERVIEW_MENU_SET_RIGHT_CLICK.getValue());
