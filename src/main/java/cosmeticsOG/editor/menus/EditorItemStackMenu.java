...
public class EditorItemStackMenu extends AbstractListMenu {
	...
	private final ItemStack emptyItem = ItemUtil.createItem(CompatibleMaterial.BARRIER.getMaterial(), 1, Message.EDITOR_MISC_EMPTY_MENU.getValue());
	...
	public EditorItemStackMenu(CosmeticsOG core, EditorMenuManager menuManager, Player owner, int particleIndex, MenuCallback callback) {
		...
		setMenu(0, Bukkit.createInventory(null, 54, Utils.legacySerializerAnyCase(Message.EDITOR_ITEMSTACK_MENU_TITLE.getValue())));
	...
	protected void build() {
		...
		ItemStack velocityItem = ItemUtil.createItem(Material.ARROW, 1, Message.EDITOR_ITEMSTACK_MENU_SET_VELOCITY.getValue());
		...
		ItemStack gravityItem = ItemUtil.createItem(Material.LEATHER_BOOTS, 1, Message.EDITOR_ITEMSTACK_MENU_TOGGLE_GRAVITY.getValue());
		...
		ItemStack durationItem = ItemUtil.createItem(CompatibleMaterial.FIREWORK_STAR.getMaterial(), 1, Message.EDITOR_ITEMSTACK_MENU_SET_DURATION.getValue());
		...
		ItemStack addItem = ItemUtil.createItem(CompatibleMaterial.TURTLE_HELMET.getMaterial(), 1, Message.EDITOR_ITEMSTACK_MENU_ADD_ITEM.getValue());
		...
		for (int i = 0; i < items.size(); i++) {
			...
			ItemUtil.setNameAndDescription(item, Utils.legacySerializerAnyCase(displayName), StringUtil.parseDescription(Message.EDITOR_ICON_MENU_ICON_DESCRIPTION.getValue()));
	...
	private void addItem (int slot, ItemStack item) {
		...
		ItemStack i = ItemUtil.createItem(material, Message.EDITOR_ICON_MENU_ITEM_PREFIX.getValue() + StringUtil.getMaterialName(material), StringUtil.parseDescription(Message.EDITOR_ICON_MENU_ICON_DESCRIPTION.getValue()));
