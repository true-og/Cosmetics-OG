...
public class EditorColorMenu extends AbstractStaticMenu {
	...
	public EditorColorMenu(CosmeticsOG core, EditorMenuManager menuManager, Player owner, int particleIndex, MenuCallback callback) {
		...
		this.inventory = Bukkit.createInventory(null, 54, Utils.legacySerializerAnyCase(Message.EDITOR_COLOUR_MENU_TITLE.getValue()));
	...
	protected void build() {
		...
		ItemStack redItem = ItemUtil.createItem(CompatibleMaterial.ROSE_RED.getMaterial(), 1, Message.EDITOR_COLOUR_MENU_SET_RED_VALUE.getValue());
		...
		ItemStack greenItem = ItemUtil.createItem(CompatibleMaterial.CACTUS_GREEN.getMaterial(), 1, Message.EDITOR_COLOUR_MENU_SET_GREEN_VALUE.getValue());
		...
		ItemStack blueItem = ItemUtil.createItem(CompatibleMaterial.LAPIS_LAZULI.getMaterial(), 1, Message.EDITOR_COLOUR_MENU_SET_BLUE_VALUE.getValue());
		...
		if (CosmeticsOG.serverVersion >= 13) {
			...
			ItemStack sizeItem = ItemUtil.createItem(CompatibleMaterial.REPEATER.getMaterial(), 1, Message.EDITOR_COLOUR_MENU_SET_SIZE.getValue());
		...
		ItemStack randomItem = ItemUtil.createItem(CompatibleMaterial.EXPERIENCE_BOTTLE.getMaterial(), Message.EDITOR_COLOUR_MENU_SET_RANDOM.getValue(), StringUtil.parseDescription(Message.EDITOR_COLOUR_MENU_RANDOM_DESCRIPTION.getValue()));
	...
	private void setColor (int slot, Color color, String colorName) {
		...
		setItem(slot, ItemUtil.createItem(material.getMaterial(), title, StringUtil.parseDescription(getColorDescription(color, title))));
