...
public class EditorResizeMenu extends AbstractStaticMenu {
	...
	public EditorResizeMenu(CosmeticsOG core, MenuManager menuManager, Player owner, EditorBaseMenu editorBaseMenu) {
		...
		this.inventory = Bukkit.createInventory(null, 27, Utils.legacySerializerAnyCase(Message.EDITOR_RESIZE_MENU_TITLE.getValue()));
	...
	protected void build() {
		...
		for (int i = 0; i < 7; i++) {
			...
			String t = title.replace("{1}", Integer.toString((i + 1) - (i > 3 ? 1 : 0))).replace(suffixInfo[0], i == 0 ? "" : suffixInfo[1]);
