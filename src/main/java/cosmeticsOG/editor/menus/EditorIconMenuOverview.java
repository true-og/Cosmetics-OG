...
public class EditorIconMenuOverview extends AbstractListMenu {
    ...
    public EditorIconMenuOverview(CosmeticsOG core, EditorMenuManager menuManager, Player owner,
    ...
    protected void build() {
        ...
        setButton(0, 10, mainItem, (event, slot) -> {
            ...
            EditorItemPromptMenu editorItemPromptMenu = new EditorItemPromptMenu(core, editorManager, owner, iconTitle,
        ...
        // Preview Icon.
        setItem(0, 48, ItemUtil.createItem(CompatibleMaterial.SUNFLOWER.getMaterial(), 1,
                Message.EDITOR_ICON_MENU_PREVIEW.getValue()));

        // Display Mode.
        ItemStack displayItem = ItemUtil.createItem(CompatibleMaterial.ROSE_RED.getMaterial(), 1,
                Message.EDITOR_ICON_MENU_SET_DISPLAY_MODE.getValue());
        EditorLore.updateDisplayModeDescription(displayItem, targetHat.getIconData().getDisplayMode(),
        ...
        setButton(0, 50, displayItem, (event, slot) -> {
            ...
            final int modeID = MathUtil.wrap(targetHat.getDisplayMode().getID() + increment,
                    IconDisplayMode.values().length, 0);
            final IconDisplayMode mode = IconDisplayMode.fromId(modeID);

            targetHat.setDisplayMode(mode);

            EditorLore.updateDisplayModeDescription(getItem(0, 50), mode,
                    Message.EDITOR_ICON_MENU_DISPLAY_MODE_DESCRIPTION);
        ...
        // Update Frequency.
        ItemStack frequencyItem = ItemUtil.createItem(CompatibleMaterial.REPEATER.getMaterial(), 1,
                Message.EDITOR_ICON_MENU_SET_UPDATE_FREQUENCY.getValue());
        EditorLore.updateFrequencyDescription(frequencyItem, targetHat.getIconUpdateFrequency(),
        ...
        setButton(0, 49, frequencyItem, (event, slot) -> {
            ...
            EditorLore.updateFrequencyDescription(getItem(0, 49), frequency,
                    Message.EDITOR_ICON_MENU_UPDATE_FREQUENCY_DESCRIPTION);
        ...
        MenuAction editAction = (event, slot) -> {
            ...
            if (event.isLeftClick()) {
                ...
                EditorItemPromptMenu editorItemPromptMenu = new EditorItemPromptMenu(core, editorManager, owner,
                        iconTitle, iconName, iconDescription, (item) ->
                        {
        ...
        for (int i = 1; i < items.size(); i++) {
            ...
            String displayName = Message.EDITOR_ICON_MENU_ITEM_PREFIX.getValue()
                    + StringUtil.capitalizeFirstLetter(itemTemplate.getMaterial().toString().toLowerCase());
            ...
            // List<TextComponent>.
            List<TextComponent> descriptionComponent = StringUtil
                    .parseDescription(Message.EDITOR_ICON_MENU_ICON_DESCRIPTION.getValue()).stream()
            ...
            List<Component> description = descriptionComponent.stream().map(textComponent -> (Component) textComponent) // Cast
                                                                                                                        // TextComponent
                                                                                                                        // to
                                                                                                                        // Component.
                    .collect(Collectors.toList());
