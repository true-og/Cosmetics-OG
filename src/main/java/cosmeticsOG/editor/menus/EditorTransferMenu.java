...
public class EditorTransferMenu extends AbstractStaticMenu {
	...
	List<TextComponent> textComponents = StringUtil.parseDescription(Message.EDITOR_MOVE_MENU_MOVE_DESCRIPTION.getValue())
			.stream()
			.map(component -> (TextComponent) component) // Convert list of Components to list of TextComponents.
			.collect(Collectors.toList());

	private final ItemStack emptyItem = ItemUtil.createItem(CompatibleMaterial.LIGHT_GRAY_STAINED_GLASS_PANE.getMaterial(), 1, Message.EDITOR_MOVE_MENU_MOVE.getValue(), textComponents);
	...
	public EditorTransferMenu(CosmeticsOG core, EditorMenuManager menuManager, Player owner, String menuName) {
		...
		this.inventory = Bukkit.createInventory(null, menuInventory.getSize(), Utils.legacySerializerAnyCase(EditorLore.getTrimmedMenuTitle(menuInventory.getTitle().content(), Message.EDITOR_MOVE_MENU_TITLE)));
	...
	protected void build() {
		...
		for (int i = 0; i < menuInventory.getSize(); i++) {
			...
			if (item == null) {
				...
				ItemUtil.setNameAndDescription(item, Utils.legacySerializerAnyCase(Message.EDITOR_MOVE_MENU_OCCUPIED.getValue()), StringUtil.parseDescription(Message.EDITOR_MOVE_MENU_OCCUPIED_DESCRIPTION.getValue()));
