...
public class EditorBaseMenu extends AbstractStaticMenu {
	...
	public EditorBaseMenu(CosmeticsOG core, MenuManager menuManager, Player owner, MenuInventory menuInventory) {
		...
		this.emptyItem = ItemUtil.createItem(CompatibleMaterial.LIGHT_GRAY_STAINED_GLASS_PANE.getMaterial(), Message.EDITOR_EMPTY_SLOT_TITLE.getValue(), StringUtil.parseDescription(Message.EDITOR_SLOT_DESCRIPTION.getValue()));
		this.editingTitle = EditorLore.getTrimmedMenuTitle(menuInventory.getName(), Message.EDITOR_BASE_MENU_TITLE);
		this.inventory = Bukkit.createInventory(null, menuInventory.getSize(), Utils.legacySerializerAnyCase(editingTitle));
	...
	public void setTitle(String title) {
		...
		Inventory replacementInventory = Bukkit.createInventory(null, inventory.getSize(), Utils.legacySerializerAnyCase(editingTitle));
	...
	public void resizeTo(int rows) {
		...
		if (this.rows != rows) {
			...
			Inventory replacementInventory = Bukkit.createInventory(null, (9 * rows), Utils.legacySerializerAnyCase(editingTitle));
	...
	private Hat createHat(int slot) {
		...
		ItemStack emptyItem = ItemUtil.createItem(CompatibleMaterial.SUNFLOWER.getMaterial(), 1, Message.EDITOR_MISC_NEW_PARTICLE.getValue());
