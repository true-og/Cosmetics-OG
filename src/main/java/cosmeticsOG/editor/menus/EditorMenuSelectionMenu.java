...
public class EditorMenuSelectionMenu extends AbstractListMenu {
    ...
    public EditorMenuSelectionMenu(
            CosmeticsOG core,
            EditorMenuManager menuManager,
            Player owner,
            boolean transferringHat,
        ...
        this.totalPages =
                MathUtil.calculatePageCount(core.getDatabase().getMenus(false).size(), 28);
    ...
    public void rebuild() {
        ...
        if (pages > totalPages) {
            ...
            menu.setItem(
                    50,
                    ItemUtil.createItem(
                            CompatibleMaterial.LIME_DYE.getMaterial(), 1, Message.EDITOR_MISC_NEXT_PAGE.getValue()));
        }
    ...
    private Inventory createMenu(int index) {
        ...
        String menuTitle =
                title.replace("{1}", Integer.toString(index + 1)).replace("{2}", Integer.toString(totalPages));
    ...
    private void loadMenus() {
        ...
        for (Entry<String, String> entry : loadedMenus.entrySet()) {
            ...
            ItemUtil.setItemDescription(
                    item,
                    Utils.legacySerializerAnyCase(Message.EDITOR_MENU_SELECTION_MENU_DESCRIPTION
                            .getValue()
                            .replace("{1}", value)));
