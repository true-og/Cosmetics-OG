...
public class EditorMenuSelectionMenu extends AbstractListMenu {
	...
	public EditorMenuSelectionMenu(CosmeticsOG core, EditorMenuManager menuManager, Player owner, boolean transferringHat, MenuObjectCallback callback) {
	...
	private Inventory createMenu(int index) {
		...
		String menuTitle = title.replace("{1}", Integer.toString(index + 1)).replace("{2}", Integer.toString(totalPages));
		...
		if (! isTransferringHat && owner.hasPermission(Permission.COMMAND_CREATE.getPermission())) {
			...
			menu.setItem(52, ItemUtil.createItem(CompatibleMaterial.TURTLE_HELMET.getMaterial(), 1, Message.EDITOR_MENU_SELECTION_CREATE.getValue()));
	...
	private void loadMenus() {
		...
		for (Entry<String, String> entry : loadedMenus.entrySet()) {
			...
			ItemUtil.setItemDescription(item, Utils.legacySerializerAnyCase(Message.EDITOR_MENU_SELECTION_MENU_DESCRIPTION.getValue().replace("{1}", value)));
