...
import cosmeticsOG.particles.ParticleEffect.ParticleProperty;
import cosmeticsOG.particles.effects.PixelEffect;
import cosmeticsOG.particles.properties.ColorData;
...
import cosmeticsOG.particles.properties.ParticleAnimation;
import cosmeticsOG.particles.properties.ParticleLocation;
import cosmeticsOG.particles.properties.ParticleMode;
...
public class EditorLore {
    ...
    private static final CosmeticsOG core = CosmeticsOG.instance;
    ...
    public static void updateTypeDescription(ItemStack item, Hat hat) {
        ...
        if (type.supportsAnimation()) {
            ...
            animationDescription = animationDescription.replace("{1}", animation.getStrippedName()).replace("{2}",
        ...
        if (type.isCustom()) {
            ...
            if (customEffect != null) {
                ...
                custom = typeInfo[1] + Message.EDITOR_TYPE_MENU_TYPE_PREFIX.getValue().replace("{1}",
                        StringUtil.capitalizeFirstLetter(customEffect.getImageName().toLowerCase()));
        ...
        description = description.replace(typeInfo[0], type.getStrippedName() + custom)
                .replace("{2}", animationDescription).replace(toggleInfo[0], toggle);
    ...
    public static void updateTypeItemDescription(ItemStack item, ParticleType type, boolean isSelected) {
        ...
        description = description.replace(descriptionInfo[0], typeDescription).replace(suffixInfo[0], suffix)
                .replace(selectInfo[0], select).replace(selectedInfo[0], selected)
    ...
    public static void updateModeDescription(ItemStack item, ParticleMode mode, Message description) {
        ...
        String s = description.getValue().replace("{1}", modes.get(MathUtil.wrap(index - 1, size, 0)).getDisplayName())
    ...
    public static void updateDisplayModeDescription(ItemStack item, IconDisplayMode displayMode, Message description) {
        ...
        String s = description.getValue()
                ...
                .replace("{2}", displayMode.getDisplayName()).replace("{3}", displayMode.getDescription());
        ItemUtil.setItemDescription(item, StringUtil.parseDescription(s));
    ...
    public static void updateVectorDescription(ItemStack item, Vector vector, Message description) {
        ...
        String s = description.getValue().replace("{1}", Double.toString(vector.getX()))
                .replace("{2}", Double.toString(vector.getY())).replace("{3}", Double.toString(vector.getZ()));
        ItemUtil.setItemDescription(item, StringUtil.parseDescription(s));
    ...
    public static void updateOffsetDescription(ItemStack item, Hat hat) {
        ...
        String s = description.replace("{1}", Double.toString(offset.getX()))
                .replace("{2}", Double.toString(offset.getY())).replace("{3}", Double.toString(offset.getZ()))
    ...
    public static void updateColorDescription(ItemStack item, Color color, boolean random, Message description) {
        ...
        String s = description.getValue().replace("{1}", r).replace("{2}", g).replace("{3}", b);
        ItemUtil.setItemDescription(item, StringUtil.parseDescription(s));
    ...
    public static void updateFrequencyDescription(ItemStack item, int updateFrequency, Message description) {
        ...
        String s = description.getValue().replace("{1}", Integer.toString(updateFrequency)).replaceAll("\\{2.*\\}",
                suffix);
        ItemUtil.setItemDescription(item, StringUtil.parseDescription(s));
    ...
    public static void updatePriceDescription(ItemStack item, int price, Message description) {
        ...
        // String c = price > 0 ?
        // StringUtil.escapeSpecialCharacters(SettingsManager.CURRENCY.getString()) :
        // "";
        String c = price > 0 ? SettingsManager.CURRENCY.getString() : "";

        String s = description.getValue().replace(priceData[0], price == 0 ? priceData[1] : Integer.toString(price))
                .replace("{2}", c);
    ...
    public static void updateIntegerDescription(ItemStack item, int value, Message description) {
        ...
        String s = description.getValue().replace("{1}", Integer.toString(value));
        ItemUtil.setItemDescription(item, StringUtil.parseDescription(s));
    ...
    public static void updateDoubleDescription(ItemStack item, double value, Message description) {
        ...
        String s = description.getValue().replace("{1}", Double.toString(value));
        ItemUtil.setItemDescription(item, StringUtil.parseDescription(s));
    ...
    public static void updateSoundDescription(ItemStack item, Sound sound, Sound currentSound, Message description) {
        ...
        String s = description.getValue().replace(selectedParse[0], selectedSuffix);
        ItemUtil.setItemDescription(item, StringUtil.parseDescription(s));
    ...
    public static void updateSoundItemDescription(ItemStack item, Hat hat) {
        ...
        String s = description.replace(soundInfo[0], soundName).replace("{2}", Double.toString(hat.getSoundVolume()))
                .replace("{3}", Double.toString(hat.getSoundPitch())).replace(clearInfo[0], clear);
     ...
     * @param argument
     */
    public static void updateSpecificActionDescription(ItemStack item, Hat hat, ParticleAction action,
            String argument)
    {
        ...
        description = description.replace("{1}", actionDescription).replace(dataInfo[0],
                action.hasData() ? dataInfo[1] : "");
    ...
    public static void updateGenericActionDescription(ItemStack item, Hat hat) {
        ...
        description = description.replace("{1}", leftClickDescription).replace("{2}", rightClickDescription);
        ItemUtil.setItemDescription(item, StringUtil.parseDescription(description));
    ...
    private static String getParsedActionDescription(Hat hat, ParticleAction action, String argument) {
        ...
        switch (action) {
            ...
            case OPEN_MENU: {
                ...
                description = description.replace("{1}", action.getStrippedName()).replace(menuInfo[0], menu);
            ...
            case COMMAND: {
                ...
                description = description.replace("{1}", action.getStrippedName()).replace(commandInfo[0], command);
            ...
            case DEMO: {
                ...
                String s = description.replace("{1}", action.getStrippedName()).replace("{2}", formattedTime);
                return s;
    ...
    public static void updateNameDescription(ItemStack item, Hat hat) {
        ...
        String clear = hat.getName().equals(Message.EDITOR_MISC_NEW_PARTICLE.getValue()) ? "" : clearInfo[1];

        String s = description.replace("{1}", hat.getDisplayName()).replace(clearInfo[0], clear);
    ...
    public static void updateDescriptionDescription(ItemStack item, List<String> desc) {
        ...
        if (desc.isEmpty()) {
            ...
            description = description.replace("{1}", sb.toString()).replace(clearInfo[0], clearInfo[1]);
    ...
    public static void updatePermissionDescription(ItemStack item, Hat hat) {
        ...
        String s = description.replace("{1}", hat.getPermission()).replace("{2}", hat.getFullPermission());
        ItemUtil.setItemDescription(item, StringUtil.parseDescription(s));
    ...
    public static void updateLabelDescription(ItemStack item, String label) {
        ...
        String s = description.replace(labelInfo[0], l).replace(clearInfo[0], clear);
    ...
    public static void updateEquipDescription(ItemStack item, String equip) {
        ...
        String s = description.replace(equipInfo[0], e).replace(clearInfo[0], clear);
    ...
    public static void updatePermissionDeniedDescription(ItemStack item, String permissionDenied) {
        ...
        String s = description.replace(deniedInfo[0], denied).replace(clearInfo[0], clear);
    ...
    /**
     * Applies a description to this ItemStack using a hats supported tracking
     * methods
     * 
     * @param item
     * @param hat
    ...
    public static void updateTrackingDescription(ItemStack item, Hat hat) {
        ...
        if (size == 1) {
            ...
            description = desc.replace("{1}", methods.get(MathUtil.wrap(index - 1, size, 0)).getDisplayName())
        ...
        switch (property) {
            ...
            case DUST_OPTIONS: {
                ...
                if (color.isRandom()) {
                    ...
                    String s = description.replace("{1}", particleName).replace("{2}", Integer.toString(c.getRed()))
            ...
            case BLOCK_DATA: {
                ...
                String description = isBlock ? Message.EDITOR_PARTICLE_BLOCK_DESCRIPTION.getValue()
                        : Message.EDITOR_PARTICLE_ITEM_DESCRIPTION.getValue();
                // String name = isBlock ?
                // hat.getParticleBlock(particleIndex).getType().toString() :
                // hat.getParticleItem(particleIndex).getType().toString();
                String s = description.replace("{1}", particleName).replace("{2}",
                        StringUtil.capitalizeFirstLetter(name.toLowerCase()));
                ItemUtil.setItemDescription(item, StringUtil.parseDescription(s));
                break;
            ...
            case ITEMSTACK_DATA: {
                ...
                String s = description.replace("{1}", particleName).replace("{2}", Integer.toString(items));
                ItemUtil.setItemDescription(item, StringUtil.parseDescription(s));
                break;
    ...
    public static void updateParticleItemDescription(ItemStack item, ParticleEffect particle, boolean isSelected) {
        ...
        description = description.replace(selectInfo[0], select).replace(selectedInfo[0], selected)
    ...
    public static void updateBooleanDescription(ItemStack item, boolean enabled, Message message) {
        ...
        String s = description.replace(enabledInfo[0], e).replace(disabledInfo[0], d);
        ItemUtil.setItemDescription(item, StringUtil.parseDescription(s));
    ...
    public static void updateHatDescription(ItemStack item, Hat hat, boolean isHat) {
        ...
        if (hat.getLeftClickAction() == ParticleAction.EQUIP || hat.getRightClickAction() == ParticleAction.EQUIP) {
            ...
            String type = hat.getType().isCustom() ? hat.getCustomEffect().getImageDisplayName()
                    : hat.getType().getStrippedName();
            ...
            description.add(frequencyDesc.replace("{1}", Integer.toString(hat.getUpdateFrequency()))
        ...
        if (isHat) {
            ...
            switch (leftAction) {
                ...
                case COMMAND:
                    String command = Message.EDITOR_HAT_COMMAND_DESCRIPTION.getValue().replace("{1}",
                            hat.getLeftClickArgument());
                    description.add(leftActionDesc.replace("{1}", command));
                    break;

                case OPEN_MENU_PERMISSION:
                case OPEN_MENU:
                    String menu = Message.EDITOR_HAT_MENU_DESCRIPTION.getValue().replace("{1}",
                            hat.getLeftClickArgument());
                ...
                case DEMO:
                    ...
                    String durationDesc = Message.EDITOR_HAT_DURATION_DESCRIPTION.getValue().replace("{1}",
                            formattedTime);
        ...
        // Convert the List<Component> to List<String> using legacySerializerAnyCase and
        // add to description.
        ...
        // Set the item description with colorized text.
    ...
    public static void updateActiveHatDescription(ItemStack item, Hat hat) {
        ...
        String s = description.replace(activeInfo[0], active).replace(hiddenInfo[0], hidden);
    ...
    public static void updatePotionDescription(ItemStack item, PotionEffect pe) {
        ...
        String s = description.replace(typeInfo[0], name).replace("{2}", numeral);
    ...
    /**
     * Returns a menu's title trimmed to fit within an inventory's character limit.
     *
     * @param title   The title to be trimmed.
     * @param message The message used as a template.
     * @return The trimmed and color-formatted menu title.
     */
    public static String getTrimmedMenuTitle(String title, Message message) {
        ...
        // Calculate the length of the message and title.
        ...
        // Replace the extra info with the trimmed title.
