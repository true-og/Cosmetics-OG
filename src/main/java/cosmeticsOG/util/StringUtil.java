...
public class StringUtil {
    ...
    /**
     * Returns the integer value of the given string, or a default value if the
    ...
    public static String parseString(String string, Hat hat) {
        ...
        return string.replace("{TYPE}", hat.getType().getStrippedName())
                ...
                .replace("{MODE}", hat.getMode().getStrippedName()).replace("{PRICE}", Integer.toString(hat.getPrice()))
                .replace("{CURRENCY}", SettingsManager.CURRENCY.getString()).replace("<locked>", "")
                .replace("</locked>", "");
    ...
    /**
     * Parses a string for any {#=}'s and returns {#=} and its contents in an
     * array<br>
    ...
    /**
     * Attempts to interpret the boolean value of the argument Acceptable strings
     * include... (yes, no, on, off, 1, 0, true, false)
     * 
     * @param arg
     * @return
     */
