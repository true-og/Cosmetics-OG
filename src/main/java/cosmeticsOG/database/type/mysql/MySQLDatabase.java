...
public class MySQLDatabase implements Database {
	...
	{				
		...
		legacy = CosmeticsOG.serverVersion < 13;
			...
			try (Connection connection = dataSource.getConnection())
			{
				...
				sync (() -> {
					...
					if (sender.isPlayer()) {
						...
						Utils.cosmeticsOGPlaceholderMessage((Player) sender, Message.COMMAND_IMPORT_SUCCESS.replace("{1}", menuConfig.getName()));
			...
			catch (SQLException error) 
			{
				sync(() -> {
					...
					if (sender.isPlayer()) {
						...
						Utils.cosmeticsOGPlaceholderMessage((Player) sender, Message.COMMAND_IMPORT_ERROR.replace("{1}", error.getClass().getSimpleName()));

					}
					else {

						Utils.logToConsole(Message.COMMAND_IMPORT_ERROR.replace("{1}", error.getClass().getSimpleName()));
	...
	private void loadMetaData(Connection connection, String menuName, Hat hat, ItemStack item) throws SQLException {
		...
		try (PreparedStatement descriptionStatement = connection.prepareStatement(query)) {
			...
			List<Component> permissionDescriptionComponents = new ArrayList<>();
			List<Component> loreComponents = new ArrayList<>();
			IconData data = hat.getIconData();

			// Using legacySerializerAnyCase to handle any-case color codes
			ItemMeta meta = item.getItemMeta();
			meta.displayName(Utils.legacySerializerAnyCase(hat.getDisplayName()));  // Convert color-coded string to Component

			while (set.next()) {
				DataType type = DataType.fromID(set.getInt("type"));
				switch (type) {
				case DESCRIPTION:
					descriptionComponents.add(Utils.legacySerializerAnyCase(set.getString("value")));  // Handle any-case color codes
					break;

				case PERMISSION_DESCRIPTION:
					permissionDescriptionComponents.add(Utils.legacySerializerAnyCase(set.getString("value")));  // Handle any-case color codes
					break;
				...
				if (h != null)
				{
					...
					if (!set.wasNull()) 
					{
						if (legacy && itemData.contains(":"))
						{
							...
							data.setItem(ItemUtil.createItem(ItemUtil.getMaterial(itemInfo[0], Material.APPLE), 1, (int) durability));
						}
					...
					if (! set.wasNull()) 
					{
						if (legacy && blockData.contains(":"))
						{
							...
							data.setBlock(ItemUtil.createItem(ItemUtil.getMaterial(blockInfo[0], Material.STONE), 1, durability));
						}
