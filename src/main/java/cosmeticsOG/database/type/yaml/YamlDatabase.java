...
public class YamlDatabase implements Database {
    ...
    public YamlDatabase(CosmeticsOG core) {
        ...
        if (SettingsManager.LOAD_INCLUDED_CUSTOM_TYPES.getBoolean()) {
            String butterflyPath =
                    core.getDataFolder() + File.separator + "types" + File.separator + "butterfly_wings_colorable.png";
        ...
        if (SettingsManager.LOAD_INCLUDED_MENUS.getBoolean()) {
            ...
            File particlesFile =
                    new File(core.getDataFolder() + File.separator + "menus" + File.separator + "particles.yml");
    ...
    public MenuInventory getPurchaseMenu(PlayerState playerState) {
        if (purchaseConfig == null) {
            File purchaseFile =
                    new File(core.getDataFolder() + File.separator + "menus" + File.separator + "purchase.yml");
    ...
    private boolean checkPlayerConfig(CustomConfig playerConfig, UUID id) {
        ...
        int legacyPurchases =
                playerConfig.getConfig().getStringList("purchases").size();
                    ...
                    switch (particle.getProperty()) {
                        ...
                        case BLOCK_DATA: {
                            if (config.isString(path + "block-data")) {
                                hat.getParticleData(index)
                                        .setBlock(new ItemStack(ItemUtil.getMaterial(
                                                config.getString(path + "block-data"), Material.STONE)));
                            } else {
                                Material material = ItemUtil.getMaterial(
                                        config.getString(path + "block-data.id", "STONE"), Material.STONE);
                                ItemStack block = ItemUtil.createItem(
                                        material, 1, (short) config.getInt(path + "block-data.damage-value"));
                            ...
                            if (config.isString(path + "item-data")) {
                                hat.getParticleData(index)
                                        .setItem(new ItemStack(
                                                ItemUtil.getMaterial(
                                                        config.getString(path + "item-data"), Material.APPLE),
                                                1));
                            } else {
                                Material material = ItemUtil.getMaterial(
                                        config.getString(path + "item-data.id", "STONE"), Material.STONE);
                                ItemStack item = ItemUtil.createItem(
                                        material, 1, config.getInt(path + "item-data.damage-value"));
                        ...
                        case ITEMSTACK_DATA: {
                            ...
                            if (items.size() > 0) {
                                for (String itemNane : items) {
                                    itemStackData.addItem(
                                            new ItemStack(ItemUtil.getMaterial(itemNane, Material.STONE), 1));
                                }
                            }
    ...
    private void loadBaseHatData(FileConfiguration config, Hat hat, String path) {
        Material material =
                ItemUtil.getMaterial(config.getString(path + "id"), CompatibleMaterial.SUNFLOWER.getMaterial());
        if (CosmeticsOG.serverVersion < 13) {
    ...
    private void loadEssentialHatData(FileConfiguration config, Hat hat, String path, String menuName, int slot) {
        ...
        hat.setOffset(
                config.getDouble(path + "offset.x", 0),
                config.getDouble(path + "offset.y", 0),
                config.getDouble(path + "offset.z", 0));
        hat.setRandomOffset(
                config.getDouble(path + "random-offset.x", 0),
                config.getDouble(path + "random-offset.y", 0),
                config.getDouble(path + "random-offset.z", 0));
        hat.setAngle(
                config.getDouble(path + "angle.x"),
                config.getDouble(path + "angle.y"),
