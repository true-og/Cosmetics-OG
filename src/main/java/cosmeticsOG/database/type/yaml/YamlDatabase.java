...
public class YamlDatabase implements Database {
    ...
    public YamlDatabase(CosmeticsOG core) {
        ...
        if (SettingsManager.LOAD_INCLUDED_CUSTOM_TYPES.getBoolean()) {
            ...
            String butterflyPath = core.getDataFolder() + File.separator + "types" + File.separator
    ...
    public MenuInventory getPurchaseMenu(PlayerState playerState) {
        ...
        if (purchaseConfig == null) {
            ...
            File purchaseFile = new File(
                    core.getDataFolder() + File.separator + "menus" + File.separator + "purchase.yml");
    ...
    public void onLabelChange(String oldLabel, String newLabel, String menu, int slot) {
        ...
        if (label != null && newLabel != null) {
            ...
            CosmeticsOG.debug("adding label under: " + newLabel);
                    ...
                    switch (particle.getProperty()) {
                        ...
                        case COLOR: {
                            ...
                            if (config.isInt(path + "color")) {
                                ...
                                hat.getParticleData(index).getColorData()
                                        .setColor(Color.fromRGB(config.getInt(path + "color")));
                        ...
                        case BLOCK_DATA: {
                            ...
                            if (config.isString(path + "block-data")) {
                                ...
                                hat.getParticleData(index).setBlock(new ItemStack(
                                        ItemUtil.getMaterial(config.getString(path + "block-data"), Material.STONE)));

                            } else {

                                Material material = ItemUtil
                                        .getMaterial(config.getString(path + "block-data.id", "STONE"), Material.STONE);
                                ItemStack block = ItemUtil.createItem(material, 1,
                                        (short) config.getInt(path + "block-data.damage-value"));
                        ...
                        case ITEM_DATA: {
                            ...
                            if (config.isString(path + "item-data")) {
                                ...
                                hat.getParticleData(index).setItem(new ItemStack(
                                        ItemUtil.getMaterial(config.getString(path + "item-data"), Material.APPLE), 1));

                            } else {

                                Material material = ItemUtil
                                        .getMaterial(config.getString(path + "item-data.id", "STONE"), Material.STONE);
                                ItemStack item = ItemUtil.createItem(material, 1,
                                        config.getInt(path + "item-data.damage-value"));
                        ...
                        case ITEMSTACK_DATA: {
                            ...
                            if (items.size() > 0) {
                                ...
                                for (String itemNane : items) {
                                    ...
                                    itemStackData
                                            .addItem(new ItemStack(ItemUtil.getMaterial(itemNane, Material.STONE), 1));
    ...
    private void loadMetaData(FileConfiguration config, Hat hat, String path, ItemStack item) {
        ...
        if (config.contains(path + "icons")) {
            ...
            boolean legacy = CosmeticsOG.serverVersion < 13;
            ...
            for (String material : config.getStringList(path + "icons")) {
                ...
                if (legacy && material.contains(":")) {
                    ...
                    // Convert materialInfo[0] to Material and materialInfo[1] (Short) to int.
                    Material materialType = Material.getMaterial(materialInfo[0]);
                    if (materialType != null) {

                        // Add the item to iconData, casting Short to int for damage value.
                        iconData.addItem(ItemUtil.createItem(materialType, 1, (int) Short.valueOf(materialInfo[1])));

                    }

                } else {

                    // For non-legacy, create an ItemStack from the material.
    ...
    private void loadBaseHatData(FileConfiguration config, Hat hat, String path) {
        ...
        Material material = ItemUtil.getMaterial(config.getString(path + "id"),
                CompatibleMaterial.SUNFLOWER.getMaterial());
        if (CosmeticsOG.serverVersion < 13) {

            hat.setItem(ItemUtil.createItem(material, 1, (short) config.getInt(path + "damage-value")));
        ...
        hat.setLeftClickAction(
                ParticleAction.fromName(config.getString(path + "action.left-click.id"), ParticleAction.EQUIP));
        hat.setRightClickAction(
                ParticleAction.fromName(config.getString(path + "action.right-click.id"), ParticleAction.MIMIC));
    ...
    private void loadEssentialHatData(FileConfiguration config, Hat hat, String path, String menuName, int slot) {
        ...
        hat.setOffset(config.getDouble(path + "offset.x", 0), config.getDouble(path + "offset.y", 0),
                config.getDouble(path + "offset.z", 0));
        hat.setRandomOffset(config.getDouble(path + "random-offset.x", 0),
                config.getDouble(path + "random-offset.y", 0), config.getDouble(path + "random-offset.z", 0));
        hat.setAngle(config.getDouble(path + "angle.x"), config.getDouble(path + "angle.y"),
                config.getDouble(path + "angle.z"));
