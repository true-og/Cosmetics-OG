...
import cosmeticsOG.tasks.HatTask;
...
public class EntityState {
    ...
    public boolean isEquipped(String label) {
        return activeHats.stream()
                .anyMatch(hatTask -> hatTask.getHat().getLabel().equals(label));
    }
    ...
    public void addHat(Hat hat) {
        activeHats.add(new HatTask(CosmeticsOG.instance, owner, hat));
    }
    ...
    public void removeHat(Hat hat) {
        HatTask hatTask = activeHats.stream()
                .filter(task -> task.getHat().equals(hat))
                .findAny()
                .orElse(null);
        if (hatTask != null) {
