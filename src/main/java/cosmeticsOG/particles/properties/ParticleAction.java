...
public enum ParticleAction {
	...
	public void onClick (Player player, Hat hat, int slot, Inventory inventory, String argument)
	{
			...
			{					
				...
				if (playerBalance > -1)
				{
					if (playerBalance < hat.getPrice())
					{
						...
						Utils.cosmeticsOGPlaceholderMessage(player, Message.INSUFFICIENT_FUNDS.getValue().replace("{1}", currency));
		...
		{				
			...
			else
			{
				...
				{
					Utils.cosmeticsOGPlaceholderMessage(player, Message.COMMAND_ERROR_UNKNOWN_MENU.getValue().replace("{1}", argument));
					break;
				}
	...
	private boolean checkAgainstEquippedHats(Hat hat, int slot, PlayerState playerState, Inventory inventory) {
		if (playerState.isEquipped(hat)) {
			...
			List<Component> components = hat.getCachedDescription().stream()
					.map(Component::text)
