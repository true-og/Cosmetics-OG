...
public class ItemStackData {
     ...
     * 
     * @return Returns (6000 - duration) to approximate how long an item has been in
     *         the world
     */
    ...
    public void dropItem(World world, Location location, Hat hat) {
        ...
        item.setMetadata("PH_DroppedItem", new FixedMetadataValue(CosmeticsOG.instance, ""));
        item.setPickupDelay(36000); // 30 Minutes
        item.setVelocity(this.velocity);
        setItemTicksLived(item, getDurationLived());

        if (CosmeticsOG.serverVersion >= 10) {
    ...
    public boolean equals(Object o) {
        ...
        if (data.duration != duration)
            return false;
        if (!data.velocity.equals(velocity))
            return false;
        if (!data.items.equals(items))
            return false;
        if (data.hasGravity != hasGravity)
            return false;
        if (data.hasDirectionalVelocity != hasDirectionalVelocity)
            return false;
    ...
    private void setItemTicksLived(Item item, int ticksLived) {
        ...
        // The method #setTicksLived does not behave the same on versions older than
        // 1.13
