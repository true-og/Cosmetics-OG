	...
	public boolean execute(CosmeticsOG core, Sender sender, String label, ArrayList<String> args) {
		...
		if (core.getDatabase().getImages(false).containsKey(imageName)) {
			...
			if (sender.isPlayer()) {
				...
				Utils.cosmeticsOGPlaceholderMessage((Player) sender, Message.COMMAND_ERROR_TYPE_EXISTS.replace("{1}", imageName));
		...
		if (database.insertImage(imageName, image)) {
			...
			if (sender.isPlayer()) {
				...
				Utils.cosmeticsOGPlaceholderMessage((Player) sender, Message.COMMAND_ADD_TYPE_SUCCESS.replace("{1}", imageName));
