		...
		for (Entry<String, Command> cmds : commandManager.getCommands().entrySet()) {
			...
			if (command != null) {
				...
				String commandText = "&3Command: &f" + Utils.stripColors(command.getName()) + "\n" +
						"&3Description: &8" + Utils.stripColors(command.getDescription().getValue()) + "\n" +
						"&3Usage: &8" + Utils.stripColors(command.getUsage().getValue()) + "\n" +
						"&3Permission: &8" + Utils.stripColors(command.getPermission().getPermission());
